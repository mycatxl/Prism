...
func LoadEnvConfig() (*EnvConfig, error) {
	...
	// --- Directories ---
	cfg.CacheDir = envStr("RESIN_CACHE_DIR", "./.local/cache")
	cfg.StateDir = envStr("RESIN_STATE_DIR", "./.local/state")
	cfg.LogDir = envStr("RESIN_LOG_DIR", "./.local/logs")
	...
	cfg.ProbeConcurrency = envInt("RESIN_PROBE_CONCURRENCY", 32, &errs)
	...
	cfg.ResourceFetchMaxBytes = envInt("RESIN_RESOURCE_FETCH_MAX_BYTES", 32<<20, &errs)
	...
	if !hasAdminToken || strings.TrimSpace(cfg.AdminToken) == "" {
		errs = append(errs, "RESIN_ADMIN_TOKEN must be defined and non-empty; run prism init to generate private tokens")
	}
	if !hasProxyToken || strings.TrimSpace(cfg.ProxyToken) == "" {
		errs = append(errs, "RESIN_PROXY_TOKEN must be defined and non-empty; run prism init to generate private tokens")
	} else {
		if cfg.ProxyToken != "" {
	...
	validatePositive("RESIN_RESOURCE_FETCH_MAX_BYTES", cfg.ResourceFetchMaxBytes, &errs)
	if cfg.ResourceFetchMaxBytes > 256<<20 {
		errs = append(errs, "RESIN_RESOURCE_FETCH_MAX_BYTES must not exceed 256 MiB")
	}

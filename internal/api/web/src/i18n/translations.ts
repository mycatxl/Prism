...
import { WORKBENCH_TRANSLATIONS } from "./workbench";

export const APP_TITLE_ZH = "Prism · 节点工作台";
...
Note: Once enabled, requests without authentication information are rejected instead of being treated as Default platform requests.`,
  ...
  "自动 Intel 检测": "Automatic Intel Checks",
  "开启后，该订阅刷新新增的节点会自动进入 Intel 批量检测；关闭后该订阅不会自动排队检测。":
    "When enabled, nodes added by a refresh of this subscription are queued for Intel checks automatically. When disabled, this subscription never queues checks on its own.",
  "解析报告": "Parse Report",
  "解析报告按原因分组列出被丢弃的节点：原因、数量以及一个有限的名称样例。":
    "The parse report groups the dropped nodes by reason: reason, count, and a bounded sample of names.",
  "该订阅尚未解析；刷新一次后会在这里列出被丢弃的节点及原因。":
    "This subscription has not been parsed yet. Refresh it once to list the dropped nodes and their reasons here.",
  "共 {{total}} 个节点：导入 {{imported}} 个，丢弃 {{skipped}} 个。":
    "{{total}} nodes in total: {{imported}} imported, {{skipped}} dropped.",
  "没有节点被丢弃。": "No node was dropped.",
  "名称样例：{{names}}": "Name samples: {{names}}",
  "没有可显示的名称": "No name available",
  "（样例已截断）": " (samples truncated)",
  "另有 {{count}} 个被丢弃的节点只计入数量，未记录名称。":
    "{{count}} further dropped nodes are counted only; their names were not stored.",
  "另有 {{count}} 种原因只计入数量。": "{{count}} further reasons are counted only.",
  "查看完整列表": "Show the full list",
  "收起完整列表": "Hide the full list",
  "该订阅尚未解析。": "This subscription has not been parsed yet.",
  "解析报告超过 64 KiB，只保留了截断标记。":
    "The parse report exceeded 64 KiB, so only the truncation marker was kept.",
  "未命名": "Unnamed",
  "未知类型": "Unknown type",
  "开始测试": "Start test",
  "例如 12h": "e.g. 12h",
  ...
  // WP10 §4: node purity assessment (intel) surface of the node list.
  "纯净度评估": "Purity assessment",
  "纯净度评分": "Purity score",
  "评估状态": "Assessment state",
  "评估时间": "Assessed at",
  "未评估": "Not assessed",
  "等待评估": "Waiting for assessment",
  "等待纯净度评估": "Run a node inspection to score this egress",
  "不支持评估": "Assessment not supported",
  "评估已过期": "Assessment expired",
  "该节点还没有纯净度评估，运行一次节点检测后即可看到评分。":
    "This node has no purity assessment yet. Run a node inspection to score its egress.",
  "置信度": "Confidence",
  "置信度高": "High confidence",
  "置信度中": "Medium confidence",
  "置信度低": "Low confidence",
  "置信度未知": "Confidence unknown",
  "最低置信度": "Minimum confidence",
  "全部置信度": "Any confidence",
  "最低纯净度": "Minimum purity",
  "最高纯净度": "Maximum purity",
  "判定": "Verdict",
  "全部判定": "Any verdict",
  "风险标记": "Risk flags",
  "未见明显标记": "No flags",
  ...
  "是": "Yes",
  "否": "No",
  "格式 检测项:结果，如 chatgpt:available": "Format check:outcome, for example chatgpt:available",
  // WP09 §4/§5.4: the intel data source and unlock check settings page.
  "数据源与解锁检测": "Data Sources & Unlock Checks",
  "数据源与检测": "Data Sources & Checks",
  "{{count}} 个数据源 · {{checks}} 条检测规则": "{{count}} data sources · {{checks}} check rules",
  "数据源": "Data Sources",
  "解锁检测": "Unlock Checks",
  "正在加载数据源...": "Loading data sources...",
  "暂无数据源": "No data sources",
  "正在加载检测规则...": "Loading check rules...",
  "暂无检测规则": "No check rules",
  "{{count}} 个用户规则文件加载失败": "{{count}} user rule files failed to load",
  "检测开关已保存": "Check toggle saved",
  "经节点查询": "Via node",
  "服务器直连查询": "Host-side lookup",
  "已配置密钥": "Key configured",
  "缺少密钥": "Key missing",
  "已暂停": "Paused",
  "已限流至 {{time}}": "Rate limited until {{time}}",
  "今日额度已用尽": "Today's quota is exhausted",
  "队列 {{queued}} · 运行 {{running}} · 失败 {{failed}}":
    "Queue {{queued}} · running {{running}} · failed {{failed}}",
  "运行正常": "Healthy",
  "今日用量 {{used}} / {{limit}}": "Today {{used}} / {{limit}}",
  "不限": "Unlimited",
  "该数据源需要密钥才能运行，配置后会自动启用。":
    "This data source cannot run without a key; it is enabled automatically once configured.",
  "每日额度": "Daily limit",
  "0 表示不限；默认 {{def}}": "0 means unlimited; default {{def}}",
  "，上限 {{max}}": ", maximum {{max}}",
  "每秒请求数 (QPS)": "Requests per second (QPS)",
  "证据有效期 (TTL)": "Evidence TTL",
  "Go 时长格式，例如 24h；默认 {{def}}": "Go duration such as 24h; default {{def}}",
  "该数据源的凭据字段在“配置”中设置：{{fields}}":
    "The credential fields of this data source are set in the configuration: {{fields}}",
  "配置 (JSON)": "Configuration (JSON)",
  "例如 DNSBL 的 zones 与 resolver；密钥类字段只写入不显示。":
    "For example the DNSBL zones and resolver. Secret fields are write-only and never displayed.",
  "保存设置": "Save settings",
  "保存密钥": "Save key",
  "清除密钥": "Clear key",
  "解除暂停": "Resume",
  "更新 API Key（留空则保持不变）": "Update API key (leave empty to keep the current one)",
  "API Key": "API key",
  "已配置": "Configured",
  "未配置": "Not configured",
  "密钥只写入，接口永不返回；保存后立即对运行中的调度生效。":
    "The key is write-only and is never returned by the API; it applies to the running scheduler immediately.",
  "最近修改 {{time}}": "Last changed {{time}}",
  "数据源 {{id}} 已更新": "Data source {{id}} updated",
  "数据源 {{id}} 已解除暂停": "Data source {{id}} resumed",
  "每日额度必须是非负整数": "Daily limit must be a non-negative integer",
  "QPS 必须是非负数": "QPS must be a non-negative number",
  "配置必须是合法的 JSON 对象": "The configuration must be a valid JSON object",
  "配置 {{key}} 的值必须是字符串、数字、布尔值或字符串数组":
    "The value of {{key}} must be a string, number, boolean, or a list of strings",
  "启用数据源 {{id}}": "Enable data source {{id}}",
  "版本 {{version}} · 类别 {{category}} · TTL {{ttl}} · 超时 {{timeout}}":
    "Version {{version}} · category {{category}} · TTL {{ttl}} · timeout {{timeout}}",
  "用户规则文件 {{path}}": "User rule file {{path}}",
  "内置规则": "Built-in rule",
  " · 校准日期 {{date}}": " · calibrated {{date}}",
  " · 已由设置页覆盖": " · overridden by the settings page",
  "启用检测 {{id}}": "Enable check {{id}}",
  "用户规则放在 $PRISM_STATE_DIR/checks.d/*.yaml，每 60 秒热加载。":
    "User rules live in $PRISM_STATE_DIR/checks.d/*.yaml and are hot-reloaded every 60 seconds.",
};
...
export const EN_TRANSLATIONS: Readonly<Record<string, string>> = { ...EXACT_ZH_TO_EN, ...WORKBENCH_TRANSLATIONS };

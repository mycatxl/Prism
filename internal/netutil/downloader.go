...
func (e *HTTPStatusError) Error() string {
	return fmt.Sprintf("downloader: unexpected status %d from %s", e.StatusCode, resourceOrigin(e.URL))
}
...
func NewDirectDownloader(timeoutFn func() time.Duration, userAgentFn func() string) *DirectDownloader {
	...
	return &DirectDownloader{
		...
		MaxBodyBytes: DefaultResourceMaxBytes,
	}
}
...
func (d *DirectDownloader) Download(ctx context.Context, url string) ([]byte, error) {
	...
	if err != nil {
		return nil, &NonRetryableError{Err: sanitizeRequestError(err)}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("downloader: %w", sanitizeRequestError(err))
	}
	...
	body, err := readLimitedBody(resp.Body, d.MaxBodyBytes)
	if err != nil {

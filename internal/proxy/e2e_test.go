...
func TestForwardProxy_E2EHTTPSuccess(t *testing.T) {
	...
	if w.Code != http.StatusCreated {
		t.Fatalf("status: got %d, want %d (body=%q, resinErr=%q)",
			w.Code, http.StatusCreated, w.Body.String(), w.Header().Get("X-Prism-Error"))
	}
...
func TestForwardProxy_E2EHTTPBypassDialsDirect(t *testing.T) {
	...
	if w.Code != http.StatusOK {
		t.Fatalf("status: got %d, want %d (body=%q, resinErr=%q)",
			w.Code, http.StatusOK, w.Body.String(), w.Header().Get("X-Prism-Error"))
	}
...
func TestForwardProxy_E2EHTTPDialTimeout_ZeroEgress(t *testing.T) {
	...
	if w.Code != http.StatusGatewayTimeout {
		t.Fatalf("status: got %d, want %d (body=%q, resinErr=%q)",
			w.Code, http.StatusGatewayTimeout, w.Body.String(), w.Header().Get("X-Prism-Error"))
	}
	if got := w.Header().Get("X-Prism-Error"); got != "UPSTREAM_TIMEOUT" {
		t.Fatalf("X-Prism-Error: got %q, want %q", got, "UPSTREAM_TIMEOUT")
	}

	select {
	case logEv := <-emitter.logCh:
		if logEv.PrismError != "UPSTREAM_TIMEOUT" {
			t.Fatalf("PrismError: got %q, want %q", logEv.PrismError, "UPSTREAM_TIMEOUT")
		}
...
func TestReverseProxy_E2EHTTPBypassDialsDirect(t *testing.T) {
	...
	if w.Code != http.StatusAccepted {
		t.Fatalf("status: got %d, want %d (body=%q, resinErr=%q)",
			w.Code, http.StatusAccepted, w.Body.String(), w.Header().Get("X-Prism-Error"))
	}
...
func TestForwardProxy_CONNECTResponseFlushFailureDoesNotPenalizeNode(t *testing.T) {
	...
	case logEv := <-emitter.logCh:
		...
		if logEv.PrismError != ErrUpstreamRequestFailed.PrismError {
			t.Fatalf("PrismError: got %q, want %q", logEv.PrismError, ErrUpstreamRequestFailed.PrismError)
		}
...
func TestForwardProxy_CONNECTZeroTrafficMarkedFailed(t *testing.T) {
	...
	case logEv := <-emitter.logCh:
		...
		if logEv.PrismError != "UPSTREAM_REQUEST_FAILED" {
			t.Fatalf("CONNECT zero-traffic resin_error: got %q, want %q", logEv.PrismError, "UPSTREAM_REQUEST_FAILED")
		}

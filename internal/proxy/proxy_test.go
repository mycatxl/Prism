...
func TestForwardProxy_AuthFailed(t *testing.T) {
	...
	if w.Header().Get("X-Prism-Error") != "AUTH_FAILED" {
		t.Fatalf("expected AUTH_FAILED, got %q", w.Header().Get("X-Prism-Error"))
	}
}
...
func TestWriteProxyError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if w.Header().Get("X-Prism-Error") != tt.wantHeader {
				t.Fatalf("X-Prism-Error: got %q, want %q", w.Header().Get("X-Prism-Error"), tt.wantHeader)
			}
		})
	}
}
...
// Upstream Resin used X-Resin-Account as the only account header; Prism keeps
// accepting it (see the compat case below) and adds X-Prism-Account, which
// wins when both are present (deviation X4).
func TestReverseProxy_ResolveReverseProxyAccount_XPrismAccountHeaderTakesPriority(t *testing.T) {
	rp := &ReverseProxy{}
	plat := &platform.Platform{
		ReverseProxyEmptyAccountBehavior: string(platform.ReverseProxyEmptyAccountBehaviorRandom),
	}
	...
	req := httptest.NewRequest("GET", "http://example.com/", nil)
	req.Header.Set("X-Prism-Account", "acct-from-header")
	req.Header.Set("Authorization", "acct-from-rule")
...
// TestReverseProxy_ResolveReverseProxyAccount_XResinAccountHeaderCompat keeps a
// case on the upstream X-Resin-Account spelling so the compatibility alias is
// covered by the ported proxy suite as well (deviation X4).
func TestReverseProxy_ResolveReverseProxyAccount_XResinAccountHeaderCompat(t *testing.T) {
	plat := &platform.Platform{
		ReverseProxyEmptyAccountBehavior: string(platform.ReverseProxyEmptyAccountBehaviorRandom),
	}
	rp := &ReverseProxy{
	...
	// Only the legacy header is set: it must be honoured over the URL account
	// and over the header extraction rule.
	parsed := &parsedPath{Host: "example.com", Path: "v1/users", Account: "acct-from-url"}
	req := httptest.NewRequest("GET", "http://example.com/", nil)
	req.Header.Set("X-Resin-Account", "acct-from-resin-header")
	req.Header.Set("Authorization", "acct-from-rule")

	account, _, extractionFailed := rp.resolveReverseProxyAccount(parsed, req, plat)
	if account != "acct-from-resin-header" {
		t.Fatalf("legacy header account: got %q, want %q", account, "acct-from-resin-header")
	}
	if extractionFailed {
		t.Fatal("legacy header-provided account should not mark extraction as failed")
	}

	// Both headers present: the Prism spelling wins.
	both := httptest.NewRequest("GET", "http://example.com/", nil)
	both.Header.Set("X-Prism-Account", "acct-from-prism-header")
	both.Header.Set("X-Resin-Account", "acct-from-resin-header")

	account, _, extractionFailed = rp.resolveReverseProxyAccount(parsed, both, plat)
	if account != "acct-from-prism-header" {
		t.Fatalf("precedence: got %q, want %q", account, "acct-from-prism-header")
	}
	if extractionFailed {
		t.Fatal("both headers set should not mark extraction as failed")
	}
}
...
func TestReverseProxy_AccountRejection_EmptyPlatform(t *testing.T) {
	...
	if w.Header().Get("X-Prism-Error") != "ACCOUNT_REJECTED" {
		t.Fatalf("expected ACCOUNT_REJECTED, got %q", w.Header().Get("X-Prism-Error"))
	}
}
...
func TestMapRouteError_PlatformNotFound(t *testing.T) {
	...
	if pe.HTTPCode != 404 || pe.PrismError != "PLATFORM_NOT_FOUND" {
		t.Fatalf("expected 404 PLATFORM_NOT_FOUND, got %d %s", pe.HTTPCode, pe.PrismError)
	}
}

func TestMapRouteError_NoAvailableNodes(t *testing.T) {
	pe := mapRouteError(routing.ErrNoAvailableNodes)
	if pe.HTTPCode != 503 || pe.PrismError != "NO_AVAILABLE_NODES" {
		t.Fatalf("expected 503 NO_AVAILABLE_NODES, got %d %s", pe.HTTPCode, pe.PrismError)
	}
}
...
func TestStripForwardingIdentityHeaders(t *testing.T) {
	...
	header.Set("X-Prism-Account", "debug-account")
...
func TestFullErrorTable(t *testing.T) {
	// Verify all error constants have correct HTTP codes and X-Prism-Error values.
	...
	for pe, want := range errorTable {
		if pe.HTTPCode != want.code {
			t.Errorf("%s: HTTPCode=%d, want %d", pe.PrismError, pe.HTTPCode, want.code)
		}
		if pe.PrismError != want.header {
			t.Errorf("ResinError=%q, want %q", pe.PrismError, want.header)
		}
	}
}

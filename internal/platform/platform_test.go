...
func noQualityLookup(addr netip.Addr) quality.Summary { return quality.Summary{} }
...
func TestPlatform_EvaluateNode_AllPass(t *testing.T) {
	...
	p.FullRebuild(func(fn func(node.Hash, *node.NodeEntry) bool) {
		fn(h, entry)
	}, alwaysLookup, usGeoLookup, noQualityLookup)
...
func TestPlatform_EvaluateNode_RegionFilter_PrefersStoredRegion(t *testing.T) {
	...
	p.FullRebuild(func(fn func(node.Hash, *node.NodeEntry) bool) {
		fn(h, entry)
	}, alwaysLookup, geoLookup, noQualityLookup)
...
func TestPlatform_NotifyDirty_AddRemove(t *testing.T) {
	...
	p.NotifyDirty(h, getEntry, alwaysLookup, usGeoLookup, noQualityLookup)
	if p.View().Size() != 1 {

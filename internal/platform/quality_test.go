...
// QualityPolicy used to be a package-local duplicate of the persisted policy
// type. Prism keeps a single definition in internal/model, so the tests alias
// it to keep the cases readable. The score field was renamed MinScore ->
// MinPurity while the wire name (min_score) stayed backwards compatible.
type QualityPolicy = model.QualityPolicy

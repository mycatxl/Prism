...
func TestFlushBucket_RetainsPendingTaskUntilRepoRecovers(t *testing.T) {
	...
	flushedBucketStart := time.Now().Unix() - 2
	mgr.bucket.mu.Lock()
	mgr.bucket.currentStart = flushedBucketStart
	...
	// Retrying across a clock boundary may also persist a new empty bucket.
	// Verify the failed bucket itself survived and was written exactly once.
	from, to := flushedBucketStart, flushedBucketStart

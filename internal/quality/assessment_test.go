...
func TestAssessmentUsesIPPureScoreAndIndependentRiskEvidence(t *testing.T) {
	...
	if a=Assess(summary,now);a.State!="partial" || a.Verdict!="incomplete" { t.Fatal("partial network evidence was presented as complete") }
...
func TestQueuedInspectionStateRemainsPending(t *testing.T) {
	...
	if got:=Assess(summary,time.Now());got.State!="pending" || got.PurityScore!=nil { t.Fatal("queued inspection was lost from the waiting filter") }
}

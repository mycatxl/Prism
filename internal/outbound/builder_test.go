...
func newTestSingboxBuilder(t *testing.T) *SingboxBuilder {
	...
	b, err := NewSingboxBuilderWithConfig(SingboxBuilderConfig{
		...
		// WP06: keep -race green; see SingboxBuilderConfig.QuietInterfaceMonitor.
...
func TestSingboxBuilder_ParseExtendedProtocols(t *testing.T) {
	...
	cases := []struct {
		...
		// WP06 §3: WireGuard is built as a sing-box 1.14 endpoint. Both the
		// legacy form A outbound (§1.3 conversion) and the form B envelope
...
func TestSingboxBuilder_ParseDomainServerProtocols(t *testing.T) {
	// WireGuard resolves its peer during startup. Keep protocol construction
	// independent of public DNS availability and example.com records.
	...
	server := &mDNS.Server{
		...
		Handler: mDNS.HandlerFunc(func(w mDNS.ResponseWriter, req *mDNS.Msg) {
			...
			for _, question := range req.Question {
				header := mDNS.RR_Header{Name: question.Name, Rrtype: question.Qtype, Class: mDNS.ClassINET, Ttl: 60}
				switch question.Qtype {
				case mDNS.TypeA:
					resp.Answer = append(resp.Answer, &mDNS.A{Hdr: header, A: net.ParseIP("127.0.0.1")})
				case mDNS.TypeAAAA:
					resp.Answer = append(resp.Answer, &mDNS.AAAA{Hdr: header, AAAA: net.ParseIP("::1")})
				}
			}
			_ = w.WriteMsg(resp)
		}),
	...
	case <-started:
	case err := <-serverDone:
		t.Fatalf("start local DNS: %v", err)
	}
	defer func() {
		_ = server.Shutdown()
		if err := <-serverDone; err != nil {
			t.Errorf("local DNS: %v", err)
		}
	}()
	b, err := NewSingboxBuilderWithConfig(SingboxBuilderConfig{
		DNSUpstreams:          []string{"udp://" + conn.LocalAddr().String()},
		QuietInterfaceMonitor: true,
	})
	if err != nil {
		t.Fatal(err)
	...
	cases := []struct {
		...
		// WP06 §3 / §5: a wireguard endpoint whose peer is a domain, resolved
		// through Prism's node DNS chain.
		{
...
// ExchangeAsync satisfies sing-box 1.14's adapter.DNSTransport, which added
// the async exchange entry point on top of Exchange.
func (t *staticDNSTransport) ExchangeAsync(ctx context.Context, message *mDNS.Msg, callback func(response *mDNS.Msg, err error)) {
	go func() {
...
// Reset satisfies sing-box 1.14's adapter.DNSTransport; the stub keeps no
// connections so there is nothing to drop.

...
type topologyRuntime struct {
	...
	rotator          *routing.ScheduledRotator
...
func main() {
	// `prism` on its own and `prism run` both start the service; every other
	// argument selects a subcommand (see runCLI in subcommands.go).
...
func newTopologyRuntime(
	...
	// 7. Topology objects. GeoLookup is wired to the GeoIP service; QualityLookup
	// stays nil until WP10 injects the intel-backed lookup.
	...
	// 6. Node builder: outbound.NewSingboxBuilderWithConfig today, WP06 replaces
	// it with outbound.NewSingboxRuntime.
	...
	// 9. Upstream health wiring: a new node gets an outbound plus an immediate
	// egress probe; removing a node drops its outbound, its latency table and
	// marks the persisted rows dirty.
	...
	scheduler := topology.NewSubscriptionScheduler(topology.SchedulerConfig{
		...
		// WP06 §9: the parse report of every refresh is persisted, so the API can
		// answer which nodes were dropped and why (StateRepo keeps at most 64 KiB
		// per subscription).
		OnParseReport: func(subscriptionID string, reportJSON string) {
			if err := engine.SetSubscriptionParseReport(subscriptionID, reportJSON); err != nil {
				log.Printf("Warning: persist parse report for %s: %v", subscriptionID, err)
			}
		},
	})

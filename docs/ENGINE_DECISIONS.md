...
## D-3 sing-box 上游数据竞争（`route/network.go`）：记录在案，未修复

**状态**：上游缺陷（sing-box v1.14.1），Prism **没有修复**，也没有本地补丁、fork 或 `replace`。

**日期**：2026-09-24。

**是什么**：在真实 netlink 接口监视器下运行内嵌 sing-box，并用 Go 竞态检测器（`-race`）时，
...
- 读方：接口监视器的回调路径 `notifyInterfaceUpdate`（`route/network.go:512`）派生的
  `go r.updateInterface(...)` 里读 `if !r.started`（`route/network.go:574`，函数
  `updateInterface` 从 533 行开始）。

两者之间没有锁、没有 `atomic`、没有建立 happens-before 的通道，因此按 Go 内存模型就是一个
数据竞争（`bool` 不会撕裂，但仍是未定义行为，`-race` 会稳定报出）。

**性质与可复现性**：这是**上游**代码，不是 Prism 的改动；触发与否取决于启动时序与接口事件时机
（timing-dependent）。`make verify`（含 `go test -race`）通过，
`go test -race -run TestProtocolMatrix ./internal/outbound/` 在 2026-09-24 是干净的；
**此前那次复现的用例在本修订上无法再次触发**，所以这里不声称“已复现并修好”，只记录为已知的、
时序相关的上游竞争。

**当前规避（仅测试）**：单元测试用 `SingboxBuilderConfig.QuietInterfaceMonitor` 把 netlink
监视器换成空实现，从而保持 `-race` 干净；生产路径仍然使用真实监视器，因此线上仍可能触发。

**可选项**：

1. 本地 fork sing-box 并在 `go.mod` 里用 `replace` 指向它，把 `started` 改成 `atomic.Bool`
   （读用 `Load`、写用 `Store`），升级内核时必须重新核对这个补丁；
2. 等上游修复（即该字段改成同步访问，或在 `Start` 与监视器回调之间建立顺序），期间保持现状；
3. 保持现状，把命中 `github.com/sagernet/sing-box/route` 的竞态报告视为已知上游问题（本条目）。
